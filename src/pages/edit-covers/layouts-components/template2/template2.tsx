...
import 'assets/css/template-styles/template2.css';
import TemplateBloc from '../block';
import { findStyleParameters } from 'utils/helpers';

const draggingEventClasses = {
    draggingEventLeftActive: 'template-2-block__left-block blocks yes',
    draggingEventLeft: 'template-2-block__left-block blocks',
    draggingEventRightActive: 'template-2-block__right-block__body blocks yes',
...
function Template2({ status, bookPage }: IThisProps) {
        ...
        <div
            ...
            <div className="template-2-block">
                ...
                <div className="template-2-block__right-block">
                    <TemplateBloc
                        status={status}
                        pageItem={bookPage?.params?.images?.items[1]}
                        className={
                            draggingEvent.start
                    ...
                    <TemplateBloc
                        status={status}
                        pageItem={bookPage?.params?.images?.items[2]}
                        className={
                            draggingEvent.start
                                ? draggingEventClasses.draggingEventRightActive
                                : draggingEventClasses.draggingEventRight
                    ...
                    <TemplateBloc
                        status={status}
                        pageItem={bookPage?.params?.images?.items[3]}
                        className={
                            draggingEvent.start
                                ? draggingEventClasses.draggingEventRightActive
                                : draggingEventClasses.draggingEventRight
