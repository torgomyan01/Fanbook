...
import CoverEditor from './components/editor';
...
export const fanbookDefaultName = 'fanbookDefault';
...
function FrontBlock() {
    ...
    async function startSave() {
        const FrontBlock = thisBook.pages.find(
            (page: IBookPage) => page.type === pagesTypes.coverFront
        );
        ...
        const data = {
            templateId: FrontBlock?.templateId,
            params: {
                template: {
                    style: ''
                },
        ...
        if (FrontBlock && data) {
            ...
            setSaveCover(true);
            UpdatePageBook(thisBook.id, FrontBlock.id, data).then((res) => {
                const _thisBook = { ...thisBook };
                const _frontBlock = _thisBook.pages.find(
                    (page: IBookPage) => page.type === pagesTypes.coverFront
                );
                if (_frontBlock) {
                    const _frontIndexOf = _thisBook.pages.indexOf(_frontBlock);
                    const newBack: IBookPage = res.data.data.item;

                    const _pages = [..._thisBook.pages];
                    _pages.splice(_frontIndexOf, 1);
                    _pages.push(newBack);
