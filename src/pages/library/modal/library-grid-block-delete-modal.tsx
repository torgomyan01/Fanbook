...
function GridBlockModalForDeleteModal({ files, deletedId }: IThisProps) {
    const dispatch = useDispatch();
    const User = userProfile();
    const [image, setImage] = useState<any>([]);

    useEffect(() => {
        setImage([DefaultImage, DefaultImage, DefaultImage, DefaultImage]);
    }, [files]);
    ...
    function ReturnDeleted() {
        ...
        returnRemoveAlbum(files.id).then(function () {
            setLoading(false);
            deletedId(files.id);
            dispatch(setMessageUser(UM.THANK_FOR_TRUSTING));
        });
    }
...
export default GridBlockModalForDeleteModal;
